...
void Ring_Allreduce(void* sendbuf, void* recvbuf, int n, MPI_Comm comm, int comm_sz, int my_rank)
{
    ...
    int s = n / comm_sz; // chunk size
    int target = (my_rank + 1 + comm_sz) % comm_sz;
    int source = (my_rank - 1 + comm_sz) % comm_sz;
    float* tmpbuf = new float[s];

    // reduce-scatter
    for (int i = 0; i < comm_sz - 1; ++i) {
        int send_idx = (my_rank - i + comm_sz) % comm_sz;
        int recv_idx = (my_rank - i - 1 + comm_sz) % comm_sz;
        MPI_Sendrecv(
            (float*)recvbuf + send_idx * s, s, MPI_FLOAT, target, 0,
            tmpbuf, s, MPI_FLOAT, source, 0, comm, MPI_STATUS_IGNORE
        );
        for (int j = 0; j < s; ++j)
            ((float*)recvbuf)[recv_idx * s + j] += tmpbuf[j];
    }
    delete[] tmpbuf;

    // allgather
    for (int i = 0; i < comm_sz - 1; ++i) {
        int send_idx = (my_rank + 1 - i + comm_sz) % comm_sz;
        int recv_idx = (my_rank - i + comm_sz) % comm_sz;
        MPI_Sendrecv(
            (float*)recvbuf + send_idx * s, s, MPI_FLOAT, target, 0,
            (float*)recvbuf + recv_idx * s, s, MPI_FLOAT, source, 0, comm, MPI_STATUS_IGNORE
        );
    }
}
